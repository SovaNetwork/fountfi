...
contract DirectDepositStrategyTest is BaseFountfiTest {
    ...
    function setUp() public override {
        ...
        vm.mockCall(
            address(registry), abi.encodeWithSelector(registry.conduit.selector), abi.encode(address(mockConduit))
        );
        ...
        (address strategyAddr, address tokenAddr) =
            registry.deploy(address(strategyImpl), "Direct Deposit RWA", "ddRWA", address(usdc), manager, initData);
    ...
    function test_InitializeWithZeroIssuerWallet() public {
        ...
        vm.expectRevert(IDirectDepositStrategy.InvalidIssuerWallet.selector);
        newImpl.initialize("Test Token", "TEST", address(roleManager), manager, address(usdc), 6, badInitData);
    }
