...
contract DepositCapHookTest is Test {
    ...
    function testFuzz_OnBeforeDeposit(uint256 amount1, uint256 amount2) public {
        ...
        // First deposit
        IHook.HookOutput memory output1 = hook.onBeforeDeposit(
            address(token),
            USER,
            amount1,
            RECEIVER
        ...
        // Second deposit
        IHook.HookOutput memory output2 = hook.onBeforeDeposit(
            address(token),
            USER,
            amount2,
            RECEIVER
    ...
    function test_OnBeforeWithdraw_DoesNotAffectCap() public {
        ...
        // Withdraw
        IHook.HookOutput memory output = hook.onBeforeWithdraw(
            address(token),
            USER,
            DEPOSIT_AMOUNT,
            RECEIVER,
    ...
    function testFuzz_OnBeforeWithdraw(
        address tokenAddr,
        address by,
        uint256 assets,
        address to,
